...
export const MyGlobalContext = createContext<GlobalContent>({
  context: 'Hello World', // set a default value
  setContext: () => {}
});
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div>
        ...
        <div>
          <input id="dogNameInput" name="dog" onChange={handleInputChange} value={context} />
        </div>
      </div>
      <div>
...
function SomeScreen() {
  ...
  console.log('Some Screen rerender');

  return (
    <div>
      <p>
        <strong>
          Ok, so it`&apos;`s still pretty contrived... But pretend this is the rest of your app.
        </strong>
      </p>
      <div>
        <form>
...
function AppBefore() {
  ...
  return (
    <MyGlobalContext.Provider value={{ context, setContext }}>
      <h1>My awesomely slow app</h1>
      <p>
        Type in the input. Because the children elements are new every render, React must
        re-rerender them, even if they don`&apos;`t need it.
      </p>
      <div>
        <SomeForm />
        <hr />
...
function AppWrapper({ children }: any) {
  const [context, setContext] = useState<string>('Admin');
  return (
    <MyGlobalContext.Provider value={{ context, setContext }}>
      <h1>My awesomely faster app</h1>
      <p>
        Type in the input. Because the children elements are consistent between renders, React is
        able to exit early and not re-rener those unnecessarily (so the data table doesn`&apos;`t
        get re-rendered which is why this one`&apos;`s faster).
      </p>
      <div>{children}</div>
    </MyGlobalContext.Provider>
  );
